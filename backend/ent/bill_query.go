...
type BillQuery struct {
	...
	// eager-loading edges.
	withBillstatus *BillStatusQuery
	withPayments   *PaymentQuery
	withResident   *UserQuery
	withFKs        bool
...
// QueryBillstatus chains the current query on the billstatus edge.
func (bq *BillQuery) QueryBillstatus() *BillStatusQuery {
	query := &BillStatusQuery{config: bq.config}
...
// QueryResident chains the current query on the resident edge.
func (bq *BillQuery) QueryResident() *UserQuery {
...
//  WithBillstatus tells the query-builder to eager-loads the nodes that are connected to
// the "billstatus" edge. The optional arguments used to configure the query builder of the edge.
func (bq *BillQuery) WithBillstatus(opts ...func(*BillStatusQuery)) *BillQuery {
	query := &BillStatusQuery{config: bq.config}
	for _, opt := range opts {
		opt(query)
	}
	bq.withBillstatus = query
	return bq
}
...
//  WithResident tells the query-builder to eager-loads the nodes that are connected to
// the "resident" edge. The optional arguments used to configure the query builder of the edge.
func (bq *BillQuery) WithResident(opts ...func(*UserQuery)) *BillQuery {
	query := &UserQuery{config: bq.config}
	for _, opt := range opts {
		opt(query)
	}
	bq.withResident = query
	return bq
}
...
func (bq *BillQuery) sqlAll(ctx context.Context) ([]*Bill, error) {
	var (
		...
		_spec       = bq.querySpec()
		loadedTypes = [3]bool{
			bq.withBillstatus != nil,
			bq.withPayments != nil,
			bq.withResident != nil,
		}
	)
	if bq.withBillstatus != nil || bq.withResident != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := bq.withBillstatus; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Bill)
		for i := range nodes {
			if fk := nodes[i].billstatus_id; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(billstatus.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "billstatus_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Billstatus = n
			}
		}
	}
	...
	if query := bq.withPayments; query != nil {
		...
		query.Where(predicate.Payment(func(s *sql.Selector) {
			s.Where(sql.InValues(bill.PaymentsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
	...
	if query := bq.withResident; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Bill)
		for i := range nodes {
